...
def find_and_delete_row(filename, varl_value=600, varw_value=700, color_value="MDF16mm T162"):
  project = ""
  floor = ""
  unit = ""
  id = ""
  ...
  with open(filename, 'r', encoding='cp1256', newline='', errors='replace') as csvfile, open(tmpFile, 'w', newline='', encoding='cp1256') as new_file:
    ...
    for row in reader:
      ...
      if count.isdigit():
        ...
        continue
      if not found and count > 0 and row[0] == str(varl_value) and row[2] == str(varw_value) and row[7] == color_value or \
        not found and count > 0 and row[0] == str(varw_value) and row[2] == str(varl_value) and row[7] == color_value:
